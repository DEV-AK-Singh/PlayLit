...
export default function QueueTrackCard({ track }: { track: Track }) {
  ...
  return (
    <div key={track?.id} id={"vid-"+track?.id} className="bg-white p-1 hover:shadow-lg transition-all duration-200 max-w-md cursor-pointer h-20">
      <div className="flex items-center space-x-2">
        ...
        <div className="relative w-18 h-18">
          ...
          />
          <span className="absolute bg-white bottom-0 left-0 w-full h-4 text-xs flex items-center justify-center text-black border border-black">
            {track?.duration}
          </span>
        </div>
        {/* Track Info */}
        ...
        <div className="flex items-center space-x-3 pe-3">
          <button 
            onClick={()=>{window.location.href=track?.platformUrl}}
            className={`${
              track?.platform === "spotify" ? "bg-green-500" : "bg-red-500"
            } text-white h-10 w-10 rounded-full hover:${
